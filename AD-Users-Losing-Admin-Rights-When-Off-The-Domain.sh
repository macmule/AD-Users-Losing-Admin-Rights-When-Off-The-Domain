...
####################################################################################################
#
# More information: https://macmule.com//2015/11/04/ad-users-losing-admin-rights-when-off-the-domain/
#
# GitRepo: https://github.com/macmule/AD-Users-Losing-Admin-Rights-When-Off-The-Domain
#
...
adminGroupMembership=$(/usr/bin/dscl . -read /Groups/admin | awk '/GroupMembership:/{for(i=2;i<=NF;i++){out=out" "$i}; print out}')
...
if [[ $(dsmemberutil checkmembership -U "$loggedInUser" -G admin) == 'user is a member of the group' ]]; then
	...
	if [[ $adminGroupMembership =~ "$loggedInUser" ]]; then
		# Update EA
		/bin/echo "<result>$loggedInUser: Local Admin</result>"
		# Exit script
		exit 0
	else
	...
	/bin/echo "<result>$loggedInUser: Not In Admin Groups</result>"
	# Exit script
	exit 0
fi
}
...
if [[ -z $(dsconfigad -show | awk '/Active Directory Domain/') ]]; then
	# Update EA
	/bin/echo "<result>Mac: Not Bound</result>"
	# Exit script
	exit 0
fi
...
if [[ -z $loggedInUser ]]; then
	# Update EA
	/bin/echo "<result>Mac: Not Logged In</result>"
	# Exit script
	exit 0
fi
...
if [ $(dscl . -read /Users/"$loggedInUser" | awk '/UniqueID:/{ print $NF}') -lt 1000 ]; then
	# Update EA
	/bin/echo "<result>$loggedInUser: Local Account</result>"
	# Exit script
	exit 0
fi
...
if [[ -z $(dscl /Search read /Users/"$loggedInUser" AuthenticationAuthority | awk '/NetLogon;/{ print $1 }') ]]; then
	...
	/bin/echo "<result>$loggedInUser: Not AD Authenticated</result>"
	exit 0
fi
